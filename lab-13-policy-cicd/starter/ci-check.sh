#!/usr/bin/env bash
# ci-check.sh — local equivalent of the CI policy-gate.
#
# Run from `starter/`:
#   bash ci-check.sh
#
# Exit 0 → config passes policy. Exit non-zero → blocked.
#
# In real CI/CD (GitHub Actions, GitLab CI), the same 3 commands run as a
# job step that gates `terraform apply`. See README "Đào sâu" for templates.
