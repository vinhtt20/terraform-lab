...
bởi management agent, attribute do daemon host quyết định, v.v.

**Use case hợp lệ trong lab này:** `log_opts`. Docker daemon (`docker info`)
có "Default Logging Driver" và "Default Logging Options" do admin host
cấu hình. Container nào không truyền `--log-opt` sẽ THỪA HƯỞNG default đó
— và provider sẽ thấy nó trong state. Nếu config không khai báo `log_opts`,
plan sẽ liên tục muốn xoá nó (`log_opts -> null`) và đó là **force replacement**.

Hai cách xử lý:
1. Khai báo `log_opts = { "max-file" = "5", "max-size" = "20m" }` (giá trị
   thật từ `terraform state show`). Vấn đề: cứng theo host — chia repo cho
   team máy khác → diff lại.
2. `lifecycle { ignore_changes = [log_opts] }` — bỏ qua. Provider không
   "ép" thay đổi nó. **Khuyến nghị cho lab này** và mọi trường hợp tương tự.
...
Quy tắc: chỉ ignore attribute mà chủ thể KHÁC bạn (cloud, daemon, agent)
đang quản lý.
